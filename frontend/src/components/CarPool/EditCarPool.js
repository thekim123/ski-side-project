...
import { asyncEditCarpool, editCarpool, getCarpool } from '../../action/carpool';
...
export function EditCarPool() {
    ...
    const handleSubmit = async (e) => {
        ...
        console.log(carpool)
        //dispatch(editCarpool(id, carpool));
        const dummy = await dispatch(asyncEditCarpool(carpool)).unwrap();
    ...
    useEffect(() => {
        ...
        console.log(state);
        if (t === undefined) {
            setSelectedRoute("도착지가 스키장");
            setSelectedResort(carpool.destination);
        } else {
        ...
        let negotiate = {
            ...
            departure: carpool.negotiate.departure,
