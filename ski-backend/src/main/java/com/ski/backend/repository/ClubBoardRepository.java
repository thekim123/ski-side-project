...
public interface ClubBoardRepository extends JpaRepository<ClubBoard, Long> {
    ...
    @Query(value = "select cb from ClubBoard cb join ClubUser cu on cb.clubUser.id = cu.id where cu.club.id = :clubId",
            countQuery = "select count(cb) from ClubBoard  cb join ClubUser cu on cb.clubUser.id = cu.id where cu.club.id = :clubId")
    Page<ClubBoard> findByClubId(Pageable pageable, @Param("clubId") long clubId);

    @Query(value = "select cb from ClubBoard cb join ClubUser cu on cb.clubUser.id = cu.id where cu.role = :role and cb.id = :clubBoardId and cu.user.id = :userId")
    Optional<ClubBoard> findByClubBoardId(@Param("clubBoardId") long clubBoardId, @Param("userId") long userId, @Param("role") String role);
